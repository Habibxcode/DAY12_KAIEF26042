...
Parses CLI flags (--debug, --rebuild)
Loads .env and validates GEMINI_API_KEY
Initialises Gemini model
Loads/builds the FAISS vector store
Runs the interactive loop: input → retrieve → generate → display
⚙️ Prerequisites
...
Read knowledge.txt
Split it into ~N text chunks
Call the Gemini Embedding API to vectorise all chunks
Build and save the FAISS index to faiss_store/
